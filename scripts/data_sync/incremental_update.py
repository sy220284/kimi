#!/usr/bin/env python3
"""
同花顺增量更新 - 并发加速版 (6线程)
每日收盘后更新数据库

改进点:
- 6线程并发拉取，速度提升5-8倍
- 保留节假日判断逻辑
- 批量插入数据库
- 详细的进度显示
"""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent.parent.parent))

import re
import time
import threading
from datetime import datetime, timedelta
from concurrent.futures import ThreadPoolExecutor, as_completed

import pandas as pd
import requests
from data import get_db_manager


# 配置
MAX_WORKERS = 6  # 并发线程数
RATE_LIMIT = 0.05  # 每线程请求间隔(秒)
BATCH_SIZE = 100  # 数据库批量插入大小

# 全局统计
stats_lock = threading.Lock()


def get_last_trading_day():
    """
    获取最近交易日（考虑周末 + A 股法定节假日）
    内置 2024-2026 年节假日，无需外部依赖
    """
    HOLIDAYS = {
        # 2024
        '2024-01-01', '2024-02-09', '2024-02-12', '2024-02-13', '2024-02-14',
        '2024-02-15', '2024-02-16', '2024-04-04', '2024-04-05',
        '2024-05-01', '2024-05-02', '2024-05-03', '2024-06-10', '2024-09-16', '2024-09-17',
        '2024-10-01', '2024-10-02', '2024-10-03', '2024-10-04', '2024-10-07',
        # 2025
        '2025-01-01', '2025-01-27', '2025-01-28', '2025-01-29', '2025-01-30', '2025-01-31',
        '2025-04-04', '2025-05-01', '2025-05-02', '2025-05-31',
        '2025-10-01', '2025-10-02', '2025-10-03', '2025-10-06', '2025-10-07',
        # 2026
        '2026-01-01', '2026-02-16', '2026-02-17', '2026-02-18', '2026-02-19', '2026-02-20',
        '2026-04-06', '2026-05-01', '2026-05-04', '2026-05-05', '2026-06-19',
        '2026-10-01', '2026-10-02', '2026-10-05', '2026-10-06', '2026-10-07',
    }

    today = datetime.now().date()
    candidate = today

    for _ in range(10):
        if candidate.weekday() < 5 and candidate.strftime('%Y-%m-%d') not in HOLIDAYS:
            break
        candidate -= timedelta(days=1)

    return candidate


def get_daily_kline_ths(symbol, start_date, end_date):
    """从同花顺获取日线数据"""
    try:
        url = f'http://d.10jqka.com.cn/v4/line/hs_{symbol}/01/{start_date}/{end_date}/last.js'
        headers = {
            'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
            'Referer': f'http://stockpage.10jqka.com.cn/{symbol}/'
        }

        response = requests.get(url, headers=headers, timeout=15)

        if response.status_code != 200:
            return None

        text = response.text
        match = re.search(r'quote_\w+\(({.+})\);?$', text, re.DOTALL)

        if not match:
            return None

        import json as _json
        try:
            json_data = _json.loads(match.group(1))
        except _json.JSONDecodeError:
            return None

        if 'data' not in json_data or not json_data['data']:
            return None

        lines = json_data['data'].split(';')
        records = []

        for line in lines:
            if not line:
                continue
            parts = line.split(',')
            if len(parts) >= 5:
                records.append({
                    'date': f"{parts[0][:4]}-{parts[0][4:6]}-{parts[0][6:]}",
                    'open': float(parts[1]),
                    'high': float(parts[2]),
                    'low': float(parts[3]),
                    'close': float(parts[4]),
                    'volume': int(parts[5]) if len(parts) > 5 else 0,
                    'amount': float(parts[6]) if len(parts) > 6 else 0
                })

        return pd.DataFrame(records)

    except Exception as e:
        return None


def update_single_stock(args):
    """
    更新单只股票（用于线程池）
    
    Args:
        args: (symbol, start_date, end_date, index, total)
    
    Returns:
        dict: 更新结果
    """
    symbol, start_date, end_date, index, total = args

    try:
        # 获取数据
        df = get_daily_kline_ths(symbol, start_date, end_date)

        if df is None or df.empty:
            return {'symbol': symbol, 'status': 'no_data', 'count': 0, 'index': index}

        # 添加到数据库（批量插入）
        db = get_db_manager()
        sql = """INSERT INTO market_data (symbol, date, open, high, low, close, volume, amount)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (symbol, date) DO UPDATE SET
                open = EXCLUDED.open, high = EXCLUDED.high, low = EXCLUDED.low,
                close = EXCLUDED.close, volume = EXCLUDED.volume, amount = EXCLUDED.amount"""

        params_list = [
            (symbol, row['date'], row['open'], row['high'],
             row['low'], row['close'], row['volume'], row['amount'])
            for _, row in df.iterrows()
        ]
        db.pg.execute_many(sql, params_list)

        return {'symbol': symbol, 'status': 'success', 'count': len(df), 'index': index}

    except Exception as e:
        return {'symbol': symbol, 'status': 'failed', 'error': str(e), 'index': index}


def incremental_update_concurrent(symbols=None, max_workers=MAX_WORKERS):
    """
    并发增量更新数据库
    
    Args:
        symbols: 股票列表，None则更新数据库中所有已有股票
        max_workers: 并发线程数
    """
    db = get_db_manager()

    # 1. 检查数据库最后日期
    result = db.pg.execute('SELECT MAX(date) as last_date FROM market_data', fetch=True)
    last_date = result[0]['last_date']
    print(f"📊 数据库最后日期: {last_date}")

    # 2. 确定更新范围
    today = get_last_trading_day()

    if last_date >= today:
        print(f"✅ 数据已是最新 (今天: {today})")
        return {'updated': 0, 'symbols': 0}

    start_date = (last_date + timedelta(days=1)).strftime('%Y%m%d')
    end_date = today.strftime('%Y%m%d')

    print(f"📅 更新范围: {start_date} ~ {end_date}")

    # 3. 获取股票列表
    if symbols is None:
        result = db.pg.execute('SELECT DISTINCT symbol FROM market_data', fetch=True)
        symbols = [r['symbol'] for r in result]

    print(f"🔄 准备更新 {len(symbols)} 只股票 (并发: {max_workers}线程)")
    print("=" * 60)

    # 4. 并发更新
    stats = {
        'updated': 0,
        'no_data': 0,
        'failed': 0,
        'new_records': 0
    }

    # 准备任务参数
    tasks = [(s, start_date, end_date, i + 1, len(symbols)) for i, s in enumerate(symbols)]

    start_time = time.time()
    completed = 0

    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        # 提交所有任务
        future_to_task = {executor.submit(update_single_stock, task): task for task in tasks}

        # 处理完成的任务
        for future in as_completed(future_to_task):
            result = future.result()
            completed += 1

            # 更新统计
            if result['status'] == 'success':
                stats['updated'] += 1
                stats['new_records'] += result['count']
            elif result['status'] == 'no_data':
                stats['no_data'] += 1
            else:
                stats['failed'] += 1

            # 进度显示
            if completed % 10 == 0 or completed == len(symbols):
                elapsed = time.time() - start_time
                rate = completed / elapsed if elapsed > 0 else 0
                eta = (len(symbols) - completed) / rate if rate > 0 else 0
                print(f"📈 进度: {completed}/{len(symbols)} ({completed * 100 // len(symbols)}%) | "
                      f"速率: {rate:.1f}只/秒 | 预计剩余: {eta:.0f}秒")

    # 5. 汇总
    elapsed = time.time() - start_time

    print("\n" + "=" * 60)
    print("✅ 增量更新完成")
    print("=" * 60)
    print(f"总股票数: {len(symbols)}")
    print(f"✅ 成功更新: {stats['updated']} 只")
    print(f"➖ 无新数据: {stats['no_data']} 只")
    print(f"❌ 失败: {stats['failed']} 只")
    print(f"📥 新增记录: {stats['new_records']:,} 条")
    print(f"⏱️  耗时: {elapsed:.1f} 秒 ({elapsed / 60:.1f} 分钟)")
    print(f"🚀 平均速率: {len(symbols) / elapsed:.1f} 只/秒")
    print("=" * 60)

    return stats


def test_update():
    """测试更新 - 只更新20只股票"""
    db = get_db_manager()
    result = db.pg.execute("""
        SELECT symbol FROM (
            SELECT DISTINCT symbol FROM market_data
        ) t
        ORDER BY RANDOM()
        LIMIT 20
    """, fetch=True)
    test_symbols = [r['symbol'] for r in result]

    print("🧪 测试模式 - 更新20只股票")
    return incremental_update_concurrent(test_symbols, max_workers=3)


if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='同花顺增量更新 - 并发版')
    parser.add_argument('--test', action='store_true', help='测试模式（只更新20只）')
    parser.add_argument('--symbols', nargs='+', help='指定股票代码')
    parser.add_argument('--workers', type=int, default=MAX_WORKERS,
                        help=f'并发线程数 (默认: {MAX_WORKERS})')
    args = parser.parse_args()

    print("=" * 60)
    print("🚀 同花顺增量更新 - 并发加速版")
    print("=" * 60)

    if args.test:
        test_update()
    elif args.symbols:
        incremental_update_concurrent(args.symbols, max_workers=args.workers)
    else:
        incremental_update_concurrent(max_workers=args.workers)
