...
# 统计当日进当日出
same_daytrades = [t for t in result.trades if t.status == 'closed' and t.entry_date == t.exit_date]
if same_daytrades:
    print("\n【当日进当日出问题】")
    print(f"  涉及交易: {len(same_daytrades)}笔")
    for t in same_daytrades[:3]:  # 只显示前3笔
        print(f"    {t.entry_date}: 买入¥{t.entry_price:.2f} -> 卖出¥{t.exit_price:.2f}")

# 分析目标价是否合理
print("\n【目标价vs买入价分析】")
for t in result.trades[:5]:
    target_vsentry = (t.target_price / t.entry_price - 1) * 100
    print(f"  {t.entry_date}: 买入¥{t.entry_price:.2f}, 目标¥{t.target_price:.2f} ({target_vsentry:+.1f}%)")

print("\n【核心发现】")
print("  1. 信号类型标注为'impulse'但实际结构是ABC (调整浪)")
print("  2. 目标价有时低于买入价 (不合理)")
print("  3. 大量当日交易 = 止损/止盈设置过紧或计算错误")
print("  4. 持仓期内重复信号被过滤 (executetrade检查已有仓位)")
