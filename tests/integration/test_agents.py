...
class TestWaveAnalystIntegration(unittest.TestCase):
    ...
    def test_analyze_valid_symbol(self, mock_get_data_mgr):
        """测试分析有效股票代码"""
        # Mock数据获取
        ...
        mock_data_mgr = Mock()
        mock_data_mgr.get_stock_data.return_value = mock_df
        mock_get_data_mgr.return_value = mock_data_mgr
    ...
    def test_analyze_invalid_symbol(self, mock_get_data_mgr):
        """测试无效股票代码处理"""
        mock_data_mgr = Mock()
        mock_data_mgr.get_stock_data.return_value = None
        mock_get_data_mgr.return_value = mock_data_mgr
...
class TestTechAnalystIntegration(unittest.TestCase):
    ...
    @patch('data.optimized_data_manager.get_optimized_data_manager')
    def test_full_analysis_pipeline(self, mock_get_data_mgr):
        """测试完整分析流程"""
        # 创建测试数据
