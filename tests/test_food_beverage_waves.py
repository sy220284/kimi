...
class WaveAccuracyMetrics:
    ...
    def add_prediction(self, entry_wave: str, predictednext: str, actual_outcome: str):
        ...
        # 4浪买入 -> 预期浪5上涨
        is_correct = False
        if entry_wave == 'C' and actual_outcome in ['1', 'up'] or entry_wave == '2' and actual_outcome in ['3', 'up'] or entry_wave == '4' and actual_outcome in ['5', 'up']:
            is_correct = True
...
class WavePatternBacktester:
    ...
    def detect_sellsignal(self, df: pd.DataFrame, entry_wave: str) -> dict | None:
        """
        检测卖点信号
        C浪买入 -> 等待浪1完成
        2浪买入 -> 等待浪3完成
