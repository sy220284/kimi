...
def get_all_stocks():
    ...
    sql = 'SELECT symbol FROM market_data GROUP BY symbol ORDER BY COUNT(*) DESC'
...
def main():
    ...
    for idx, symbol in enumerate(stocks):
        ...
        if (idx + 1) % 10 == 0 or (idx + 1) == len(stocks):
            print(f"  [{idx+1}/{len(stocks)}] 累计信号: {len(all_signals)}")
    ...
    if all_signals:
        ...
        if len(c_df) > 0:
            ...
            # 胜率
            c_valid = c_df[c_df['b_wave_valid'] == True]['return_5d'].dropna()
            c_invalid = c_df[c_df['b_wave_valid'] == False]['return_5d'].dropna()
            if len(c_valid) > 0:
        ...
        if len(w2_df) > 0:
            ...
            # 胜率
            w2_valid = w2_df[w2_df['wave1_valid'] == True]['return_5d'].dropna()
            w2_invalid = w2_df[w2_df['wave1_valid'] == False]['return_5d'].dropna()
            if len(w2_valid) > 0:
        ...
        df.to_csv(output_dir / 'full_database_v3.csv', index=False, encoding='utf-8-sig')
        print(f"\n💾 结果保存: tests/results/full_database_v3.csv")
