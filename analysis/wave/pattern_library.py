...
class WaveStructure:
    ...
    def get_wave(self, num: str) -> SubWave | None:
        """获取指定浪号的子浪"""
        for w in self.waves:
            if w.wave_num == num:
                return w
...
class TriangleAnalyzer:
    ...
    @staticmethod
    def identify(points: list[tuple[str, float, float]], tolerance: float = 0.05) -> dict[str, Any] | None:
        """
        识别三角形调整浪
...
class WXYAnalyzer:
    ...
    def identify(points: list[tuple[str, float, float]],
                 ...
                 max_retracement: float = 0.786) -> dict[str, Any] | None:
        """
        识别 WXY 或 WXYXZ 联合调整
        ...
        # W和Y应该大致相等 (指导原则)
        equality = min(W_length, Y_length) / max(W_length, Y_length) if W_length > 0 else 0.0
...
class SubWaveDetector:
    ...
    def analyze_nesting(self, df: pd.DataFrame, parent_wave: SubWave,
                        current_level: int = 1) -> WaveStructure | None:
        """
        分析子波浪嵌套结构
    ...
    def detect_sub_impulse(self, df: pd.DataFrame, start_idx: int, end_idx: int) -> WaveStructure | None:
        """在指定范围内检测子级别推动浪"""
        ...
        if pattern and pattern.confidence > 0.35:
            # 构建WaveStructure
            waves = []
            for _i, p in enumerate(pattern.points):
                if p.wave_num:
